...
function extra.buff_change(status,event,name,gain,buff_table)
    ...
    if S{'Commitment','Dedication'}:contains(name) then
        if gain then enable("left_ring") equip(gear_equip(name,'buff_change'))
            ...
            if auto_cap_ring then
                ring_time = get_item_next_use("Trizek Ring")
            elseif auto_xp_ring then
                ring_time = get_item_next_use("Echad Ring")
            end
        end
    end
end
...
function extra.aftercast(status,current_event,spell)
    -- if (auto_cap_ring or auto_xp_ring) and ring_time <= os.time(os.date("!*t", os.time())) then
        -- local rings = S{'Trizek Ring','Echad Ring'}
        -- if not buffactive["encumbrance"] then
            -- if auto_cap_ring and not buffactive["Commitment"] then equip({left_ring="Trizek Ring"}) disable("left_ring")
            -- elseif auto_xp_ring and not buffactive["Dedication"] then equip({left_ring="Echad Ring"}) disable("left_ring")
            -- end
        -- end
        -- if rings:contains(player.equipment.left_ring) then send_command('wait 3.0;input /item "'..player.equipment.left_ring..'" <me>')
        -- end
    -- end
end
----WS/Obi equip------------------------------------------------------------------------------------------------------------------
...
function get_item_next_use(name)--returns time that you can use the item again
    for _,v in ipairs({"inventory","sack","satchel","case","wardrobe"}) do
        local item = player[v][name]
        if item then
            return extdata.decode(item).next_use_time
        end
    end
end
