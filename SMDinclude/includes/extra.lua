...
function spell_stopper(spell)
    ...
    if spell.type == "Trust" and party.count > 1 then if player.in_combat then return true end if check_in_party(string.mgsub(spell.en, "II|%[S%]|%(UC%)|%s+", "")) then return true end end
...
function spell_range_check(spell)
    ...
    if spell_max_distance < spell.target.distance then --[[print("Target out of "..spell.name.."'s max range of "..spell_max_distance)]] return true end
end
function gearchang_stopper(spell)
    ...
    if ((S{"pet_midcast","pet_aftercast"}:contains(_global.current_event) and false or Watch_pet_midaction) and pet_midaction() or false) or ((S{"midcast","aftercast"}:contains(_global.current_event) and false or Watch_midaction) and midaction() or false) then return true end
end
--extra functions-----------------------------------------------------------------------------------------------------------------
extra = {}
...
if player.main_job == "DNC" or player.sub_job == "DNC" then
    ...
    function extra.waltz_potency(s)
        local potency = 100
        local tpreduction = 0
        ...
        if potency > 130 then potency = 130 end
        return s,potency,received_pot,tpreduction
    end
    function extra.select_waltz(bpotency,received_pot,tpreduction) -- returns the most usable Waltz for use on your player
        local potency = (bpotency  + (player.main_job == "DNC" and (player.job_points.dnc.waltz_potency*2) or 0) + Contradance_potency)
        local hp_dif = (player.max_hp - player.hp) local abilitys = res.job_abilities local base = (Player['VIT'] + Player['VIT+']) + (Player['CHR'] + Player['CHR+'])
        local dnc_type local h_waltz local h_hp
        if hp_dif == 0 then return nil,0 end
        local waltz = {main_job={[1]=0.250,[2]=0.500,[3]=0.750,[4]=1.000,[5]=1.25,},sub_job={[1]=0.125,[2]=0.250,[3]=0.375,},
        ...
        for i,v in ipairs(waltz.default) do
            if player[dnc_type..'_level'] >= v.lvl then
                local hHP=math.floor((((base*waltz[dnc_type][i])+v.c)*(potency/100))*(received_pot/100)) local hTP=(v.tp-tpreduction)
                if hHP >= hp_dif and player.tp >= hTP then return abilitys[v.id][gearswap.language] elseif player.tp >= hTP then h_waltz = v.id end
            end
        end
...
if player.main_job == "WHM" or player.sub_job == "WHM" then
    function extra.cure_pot(s)--returns sets potency +%, received potency +%, cast time -%
        local potency = 100 local received_pot = 100 local rapture_potency = 50 local cast_time_reduction = 100
        ...
        for _,v in pairs(s) do
            if type(v) == 'table' then
                ...
                potency = potency + (cure_potency[v2.name] or 0) received_pot = received_pot + (received_cure_potency[v2.name] or 0)
                cast_time_reduction = cast_time_reduction - (cure_cast_time_reduction[v2.name] or 0) rapture_potency = rapture_potency + (rapture_potency_plus[v] or 0)
            end
        end
        if potency > 150 then potency = 150 end
        return s,(Rapture and rapture_potency or 0)+potency,received_pot,cast_time_reduction
    end
    function extra.select_cure(pot,rec_pot,target)
        local potency = pot + (Divine_seal and 100 or 0)
        local rec_potency = rec_pot
        local w_potency = 100
        if world.day == 'Lightsday' or world.day == "光" then w_potency = w_potency + 10
        elseif world.day == 'Darksday' or world.day == "闇" then w_potency = w_potency - 10
        end
        if world.weather_element == 'Light' or world.weather_element == '光' then w_potency = w_potency + (world.weather_id == 16 and 10 or 25)
        elseif world.weather_element == 'Dark' or world.weather_element == '闇' then w_potency = w_potency + (world.weather_id == 18 and 10 or 25)
        end
        local get_base_hp = function(name)
                ...
                local p_floor
                local cure_tbl
                for i,v in pairs(cure_map[name]) do if i <= power then p_floor = i cure_tbl = v end end
                local cure local base = floor((power-p_floor)/cure_tbl.rate)+cure_tbl.hp
                if cure_tbl.cap and cure_tbl.cap <= base then cure = cure_tbl.cap else cure = base end
                return cure
            end
        local hp_dif = (player.max_hp - player.hp) local s_cure local hp = 0 local spellt = {} local cure_spells = {1,2,3,4,5,6}
        for _,v in pairs(cure_spells) do
            local spell = res.spells[v] local hHP = get_base_hp(spell.en) local mjob = player.main_job_id local sjob = player.sub_job_id
            if spell.levels[mjob] then
                if spell.mp_cost <= player.mp and spell.levels[mjob] <= player.main_job_level then spellt[spell[gearswap.language]] = hHP
                elseif spell.mp_cost <= player.mp and spell.levels[sjob] <= player.main_job_level then spellt[spell[gearswap.language]] = hHP
                end
            elseif spell.levels[sjob] then
                if healed_hp_final >= hp_dif and spell.mp_cost <= player.mp and spell.levels[sjob] >= player.sub_job_level then spellt[spell[gearswap.language]] = hHP
                elseif spell.mp_cost <= player.mp and spell.levels[sjob] >= player.sub_job_level then spellt[spell[gearswap.language]] = hHP
                end
            end
        end
        for i,v in pairs(spellt) do
            local hHP = (v*(potency/100))*(w_potency/100) if target == 'SELF' then hHP = (hHP*(rec_potency/100)) end if hHP >= hp_dif then return i elseif hHP > hp then hp = hHP s_cure = i end
        end
        return s_cure
    end
end
function ws_to_aoews(spell)
    for _,v in pairs(table.reverse(windower.ffxi.get_abilities().weapon_skills)) do
        local ws = res.weapon_skills[v][gearswap.language] if aoe_ws:contains(ws) then return ws end
    end
    return spell.name
end
function extra.pretarget(status,event,spell)
    if spell.type == "WeaponSkill" then
        if Enable_auto_WS_aoe and aggro_count() >= (User_aggro_aoe and User_aggro_aoe or 2) and not S{"Archery","Marksmanship","Throwing"}:contains(spell.skill) then
            local new_ws = ws_to_aoews(spell) if spell.name ~= new_ws then status.end_event=true status.end_spell=true send_command('input /ws "'..new_ws..'" <t>') return end
        end
    end
end
function extra.precast(status,event,spell)
    if spell.type == 'Waltz' then
        if spell.target.type == "SELF" and spell.en:startswith('Curing Waltz') then
            local set,potency,received_pot,tpreduction = extra.waltz_potency(sets["Waltz"])
            local new_waltz,h_total = extra.select_waltz(potency,received_pot,tpreduction) 
    ...
    elseif S{"Cure","Cure II","Cure III","Cure IV","Cure V","Cure VI"}:contains(spell.en) then
        local set,potency,received_pot,cast_time_reduction = extra.cure_pot(set_combine(cure_gear[lang[gearswap.language]], sets.cure))
        local new_cure = extra.select_cure(potency,received_pot,spell.target.type)
        if new_cure and spell.en ~= new_cure then send_command('input /ma "'..new_cure..'" '..spell.target.raw) status.end_event=true status.end_spell=true end
        sets.building[event] = set_combine(sets.building[event], set)
...
function extra.buff_change(status,event,name,gain,buff_table)
    ...
    elseif name == "Reive Mark" and not gain then if reg_event and reg_event.clear_aggro_count then reg_event.clear_aggro_count:schedule(1.5) if updatedisplay then updatedisplay:schedule(1.5) end end
    end
    if S{'Commitment','Dedication'}:contains(name) and auto_ring then if gain then enable("left_ring") equip(gear_equip(name,'buff_change')) else schedule_xpcp_ring() end end
end
function extra.self_command(status,event,com)
    if type(com) == 'table' then
        ...
        elseif com[1]:lower() == 'clear' then
            if com[2]:lower() == 'aggrocount' and reg_event.atacking_mobs then if reg_event and reg_event.clear_aggro_count then reg_event.clear_aggro_count() end end
        end
    else
...
function extra.aftercast(status,current_event,spell)
    if auto_ring then if rings:contains(player.equipment.left_ring) then send_command('wait 3.0;input /item "'..player.equipment.left_ring..'" <me>') end end
end
----WS/Obi equip------------------------------------------------------------------------------------------------------------------
WS_Gear = {}
function WS_Gear.precast(status,event,spell)
    if spell.type == "WeaponSkill" then
        ...
        elseif player.inventory[sets.ws_neck[spell_element.en].neck] or player.wardrobe[sets.ws_neck[spell_element.en].neck] then sets.building[event] = set_combine(sets.building[event], sets.ws_neck[spell_element.en])
        end
        if player.inventory["Fotia Belt"] or player.wardrobe["Fotia Belt"] then sets.building[event] = set_combine(sets.building[event], {waist="Fotia Belt"})
        ...
        if ws_head and (player.inventory[sets.WS_types[spell.skill].head] or player.wardrobe[sets.WS_types[spell.skill].head]) then sets.building[event] = set_combine(sets.building[event], sets.WS_types[spell.skill]) end
    end
end
...
function get_item_next_use(name)--returns time that you can use the item again
    for _,n in pairs({"inventory","wardrobe"}) do for _,v in pairs(gearswap.items[n]) do if type(v) == "table" and v.id ~= 0 and res.items[v.id].en == name then return extdata.decode(v) end end end
end
function xp_cp_ring_equip(ring) if auto_ring then enable("left_ring") gearswap.equip_sets('equip_command',nil,{left_ring=ring}) disable("left_ring") end end
function schedule_xpcp_ring()
    local ring_time = os.time(os.date("!*t", get_item_next_use(rings[rings_count]).next_use_time))-os.time()
    if type(xpcpcoring) == "thread" then coroutine.close(xpcpcoring) end xpcpcoring = coroutine.schedule(xp_cp_ring_equip:prepare(rings[rings_count]),(ring_time > 0 and ring_time or 1))
end
function check_in_party(name) for pt_num,pt in ipairs(alliance) do for pos,party_position in ipairs(pt) do if party_position.name == name then return true end end end end
function check_ring_buff()
    local xpcprings = {cp=S{"Vocation Ring","Trizek Ring","Capacity Ring"},xp=S{"Undecennial Ring","Decennial Ring","Allied Ring","Novennial Ring","Kupofried's Ring","Anniversary Ring","Emperor Band","Empress Band","Chariot Band","Duodec. Ring","Expertise Ring"}}
    ...
    elseif xpcprings.cp:contains(rings[rings_count]) and not buffactive['Commitment'] then return true end return false
end
function c_equip(delay, set) return gearswap.equip_sets:schedule(delay, 'equip_command', nil, set) end
