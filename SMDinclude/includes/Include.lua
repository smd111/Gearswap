...
function run_event(status,a,b,c)
    ...
    if type(a) == "table" and sets[event] then local types = {'action_type','skill',--[['type',]]'name'} for i=1,#types do if sets[event][a[types[i]]] then sets.building[event] = set_combine(sets.building[event], sets[event][a[types[i]]]) end end end
    for i, v in ipairs({"fdebug","s_weapon","ninja_wheel","ammo","msi","conq_gear","extra","mji","sji","WS_Gear","e_obi","mf"}) do
        ...
        if S{"filtered_action","pretarget","precast"}:contains(event) and status.end_spell then cancel_spell() end if status.end_event then return end
    end
    ...
    elseif not status.stop_swapping_gear then
        ...
        if z and z >= 0 then c_equip(z, sets.building[event])
...
function get_sets()
    ...
    if update_display then update_display:schedule(1.5) end if mf.file_load then mf.file_load:schedule(1.5) end if file_write and file_write.write then file_write.write() end
    --if auto_ring and type(xpcpcoring) ~= "thread" and check_ring_buff() then schedule_xpcp_ring:schedule(8) end
end
...
function pretarget(spell)
    local status = {end_event=false,end_spell=false,stop_swapping_gear=true} if spell_stopper(spell) then cancel_spell() end run_event(status,spell)
end
...
function aftercast(spell)
    local status = {end_event=false,stop_swapping_gear=false} if gearchang_stopper(spell) then return end run_event(status,spell)
    if player.in_combat and auto_use_shards then local a = T{'C.','Z.','A.','P.'} local b = T{'I','II','III','IV','V'} for i=1,#a do for i2=1,#b do if player.inventory[a[i]..' Ygg. Shard '..b[i2]] then send_command('wait 3.0;input /item "'..a[i]..' Ygg. Shard '..b[i2]..'" <me>') end end end end
end
function status_change(new,old)
    local status = {end_event=false,stop_swapping_gear=false}
    if player.main_job == "THF" and reg_event and not reg_event.treasure_hunter[player.target.id] then thief_sub = thieftype['hunter'] end run_event(status,new,old)
end
...
function buff_change(name,gain,buff_table)
    ...
    elseif buff_table.id == 582 then Contradance_potency = (gain and 100 or 0) elseif buff_table.id == 267 then allied_tags = (gain and true or false)
    elseif buff_table.id == 364 then Rapture = (gain and true or false) elseif buff_table.id == 78 then Divine_seal = (gain and true or false)
    elseif buff_table.id == 511 then Reive_mark = (gain and true or false) elseif buff_table.id == 257 then Besieged = (gain and true or false)
    end
...
function sub_job_change(new,old) --[[gearswap.refresh_user_env() local status = {end_event=false,stop_swapping_gear=true} run_event(status,new,old)]] menu_commands("{regs|Reload Gearswap}") end
...
function file_unload(new_job)
    mf.file_unload(new_job) if file_write and file_write.write then file_write.write() end
...
function load_includes()
    ...
    for i, v in pairs(includes_have) do if v[1] then include(v[2]) end end
end
