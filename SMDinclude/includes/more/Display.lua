menu.text = {font='Segoe UI Symbol',size=9} menu.bg = {alpha = 200} menu.flags = {draggable=true}
...
function initialize(text, settings, name)
    ...
    if name == 'window' then
        ...
        elseif menu_set == 4 then
            ...
            if auto_ring then line:append('Ring = ${xpcpring}') end
            line:append('Auto Shard Use   ${ashard}')
            if Registered_Events then line:append('-Show-\n  MJob and LVL   ${tmjl}\n  Skill Level   ${tskill}\n  Aggro Count  ${aggroenable}') end
            if Debug then line:append('Show Debug  ${dbenable}') if full_debug then line:append('  in = ${debugtype}') end end
    ...
    elseif name == 'button' then
        ...
        local menu_initialize = {['weapon_select_window']={[1]='Select Weapon',[2]='weapon_types',[3]='wep',[4]='weapon'},
            ['range_select_window']={[1]='Select Range',[2]='range_types',[3]='rang',[4]='range'},['skill_select_window']={[1]='Select Skill',[2]='reg_event.skill_type',[3]='reg_event.skill',},
            ['debug_select_window']={[1]='Select Debug',[2]='fdebug.type',[3]='debug',},['tab_select_window']={[1]='Select Menu Tab',[2]='tab_type',[3]='tab',},
            ['gear_select_window']={[1]='Select Armor',[2]='armor_types',[3]='ger',[4]='armor'},['xpcpring_select_window']={[1]='Select CP XP Ring',[2]='rings',[3]='ring'},}
        line:append(menu_initialize[name][1])
        for i, v in ipairs(s_to_t(menu_initialize[name][2])) do
...
function updatedisplay()
    ...
    local i = {}
    if Conquest_Gear and conquest then i.cneckc = conquest.neck.change and c_m or n_c i.cneck = conq_gear.neck.case[conquest.neck.case_id] i.cringc = conquest.ring.change and c_m or n_c i.cring = conq_gear.ring.case[conquest.ring.case_id] end
    ...
    if reg_event and reg_event.skill and Registered_Events then i.skill = reg_event.skill_type[skill_count] i.skill_lvl = (reg_event.skill[reg_event.skill_type[skill_count]..' Capped'] and "Capped" or reg_event.skill[reg_event.skill_type[skill_count]..' Level']) end
    if Debug and fdebug then i.debugtype = string.gsub(fdebug.type[fdebug.count], "_", " ") end
    if Ninw and ninja_wheel then i.ninstartele = ninja_wheel.element_list[ninja_wheel.element_count] i.ninstartlvl = ninja_wheel.start_lvl i.ninstart = (ninja_wheel.super_tog or ninja_wheel.tog) and '\\cs(0,255,0)Started\\cr' or '\\cs(255,255,0)Stoped\\cr' i.ninstarttype = ninja_wheel.start_type and 'Super' or 'Normal' end
    i.stepm = Stepmax
    ...
    if rings_count > #rings then rings_count = 1 end
...
function get_window_pos(x,y,check)
    ...
    for i, v in pairs(check) do if v[1] then mx, my = texts.extents(_G[v[2]]) button_lines = _G[v[2]]:text():count('\n') + 1 hx = (x - _G[i].pos.x) hy = (y - _G[i].pos.y) end end
    ...
    for i, v in ipairs(loc) do
        if (y > v.ya and y < v.yb) then
            if type(switch[name][i]) == 'table' and not hide_button:contains(switch[name][i][1]) then
                windower.prim.set_position('window_button', _G[tab].pos.x, (_G[tab].pos.y + v.ya)) windower.prim.set_size('window_button', x, (v.yb - v.ya)) windower.prim.set_visibility('window_button', true)
            else
...
function set_count(tab,switch)
    for i,v in ipairs(s_to_t(tab[2])) do 
        for w in string.gmatch(switch, '|(.-)}') do if string.gsub(w, "_", " ") == string.gsub(v, "_", " ") then 
        if tostring(string.gmatch(switch, '{(.-)|')):startswith("ring") and get_item_next_use(rings[rings_count]) then schedule_xpcp_ring() end
        return i elseif "Custom Menu" == w then return 6 end end
    end
    
end
function set_loc(loc,hy,name,tab)
    for i, v in ipairs(loc) do
        if (hy > v.ya and hy < v.yb) then if type(switch[name][i]) == 'table' then if switch[name][i][1] then return set_count(tab,switch[name][i][1]) end end end
    end
end
function extra_display(x,y,loc,hy,check)
    local code = {['weapon_select_window']={[1]='Select Weapon',[2]='weapon_types',[3]='wep',[4]='weapon'},['range_select_window']={[1]='Select Range',[2]='range_types',[3]='rang',[4]='range'},
        ['skill_select_window']={[1]='Select Skill',[2]='reg_event.skill_type',[3]='reg_event.skill',},['debug_select_window']={[1]='Select Debug',[2]='fdebug.type',[3]='debug',},
        ['tab_select_window']={[1]='Select Menu Tab',[2]='tab_type',[3]='tab',},['gear_select_window']={[1]='Select Armor',[2]='armor_types',[3]='ger',[4]='armor'},
        ['xpcpring_select_window']={[1]='Select Xp Cp Ring',[2]='rings',[3]='ring'},}
    for i, v in pairs(check) do
        if v[1] then
            local tab = {}
            for k, v in  string.gmatch(v[3], "([%w_]+)%.([%w_]+)") do tab[1] = k tab[2] = v end
            if _G[v[3]] then _G[v[3]] = set_loc(loc,hy,v[2],code[v[2]]) or 1
            else _G[tab[1]][tab[2]] = set_loc(loc,hy,v[2],code[v[2]]) or 1
            end
            kill_window(v[2]) switch[v[2]] = nil windower.prim.set_visibility('window_button', false)
...
function c_w(a,b,x,y)
    return (_G[a] and _G[b] and _G[b]:hover(x, y) and _G[b]:visible()) or (_G[b] and _G[b]:hover(x, y) and _G[b]:visible()) or (type(_G[a]) == "table" and _G[a]:hover(x, y) and _G[a]:visible())
end
function mouse(mtype, x, y, delta, blocked)
    local check_windows = {
    ['menu']={[1]=c_w('window',nil,x,y),[2]='window'},['min_window']={[1]=c_w('button',nil,x,y),[2]='button'},
    ['skill_select']={[1]=c_w('skillwatch','skill_select_window',x,y),[2]='skill_select_window',[3]='skill_count'},
    ['debug_select']={[1]=c_w('Debug','debug_select_window',x,y),[2]='debug_select_window',[3]='fdebug.count'},
    ['tab_select']={[1]=c_w(nil,'tab_select_window',x,y),[2]='tab_select_window',[3]='menu_set'},
    ['wep_select']={[1]=c_w(nil,'weapon_select_window',x,y),[2]='weapon_select_window',[3]='weapon_types_count'},
    ['range_select']={[1]=c_w(nil,'range_select_window',x,y),[2]='range_select_window',[3]='range_types_count'},
    ['ger_select']={[1]=c_w(nil,'gear_select_window',x,y),[2]='gear_select_window',[3]='armor_types_count'},
    ['ring_select']={[1]=c_w(nil,'xpcpring_select_window',x,y),[2]='xpcpring_select_window',[3]='rings_count'},}
    ...
    if mtype == 0 then
        if button:hover(x, y) and button:visible() then
            ...
            for i, v in pairs(check_windows) do if v[1] then set_prim_loc(location,v[2],i,mx,hy) end end
        end
    elseif mtype == 2 then
...
function menu_commands(com)
    ...
    elseif com == "{hid|HIDE}" then window:hide() window_hidden = true button:show() windower.prim.set_visibility('window_button', false)
        local kill={"gear_select_window","tab_select_window","skill_select_window","weapon_select_window","range_select_window","debug_select_window","xpcpring_select_window"}
        for i,v in ipairs(kill) do if _G[v] then kill_window(v) end end
    elseif com == "{wsaoe}" then Enable_auto_WS_aoe = not Enable_auto_WS_aoe
    elseif com == "{filew|Force File Write}" then file_write.write()
    elseif com == "{reagro}" then reg_event.clear_aggro_count()
    elseif com == "{axpcpring}" then auto_ring = not auto_ring
    elseif com == "{wshead}" then ws_head = not ws_head
    ----Gearswap Controls----
    ...
    elseif com == "{gsex1|Gearswap Export}" then gearswap.export_set({'mainjob','overwrite'})
        ...
        if S{"{amode}","{listm}","{skill}","{wept}","{rwept}","{debugtype}","{xpcpring}"}:contains(com) then
            ...
            local menu_open ={["{amode}"]={[1]="gear_select_window",[2]="ger_select",[3]=ctext,},["{listm}"]={[1]="tab_select_window",[2]="tab_select",[3]=ctext},
                ["{wept}"]={[1]="weapon_select_window",[2]="wep_select",[3]=ctext},["{rwept}"]={[1]="range_select_window",[2]="range_select",[3]=ctext},
            ["{debugtype}"]={[1]="debug_select_window",[2]="debug_select",[3]=ctext},["{xpcpring}"]={[1]="xpcpring_select_window",[2]="ring_select",[3]=ctext},
            ...
            for i,v in pairs(menu_open) do
                if com == i then
                    if not _G[v[1]] then _G[v[2]] = v[3] _G[v[1]] = texts.new(_G[v[2]])
                        initialize(_G[v[1]], _G[v[2]], v[1]) _G[v[1]]:show()
                    else kill_window(v[1])
                    end
                    if com == "{xpcpring}" then schedule_xpcp_ring() end
                elseif _G[v[1]] then kill_window(v[1]) end
            end
        end
        ----Include Enable/Disable Rules----
        local include_switch = {
            ["{tmji}"]={[1]="MJi",[2]='mjob/main_job_'..player.main_job..'.lua',[3]="mji"},["{tsji}"]={[1]="SJi",[2]='sjob/sub_job_'..player.sub_job..'.lua',[3]="sji"},
