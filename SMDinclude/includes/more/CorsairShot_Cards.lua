...
function card.check(name)
    ...
    if player.inventory[card_case[gearswap.language]] ~= nil then
        return card_case[gearswap.language]
    elseif player.inventory[gearswap.res.items[5870][gearswap.language]] ~= nil then
        return gearswap.res.items[5870][gearswap.language]
    end
end
function card.rule(status,current_event,spell)
    if spell.type == "CorsairShot" then
        ...
        if name then
            card.item_use = name send_command('input /item "'..name..'" <me>')
        end
        status.end_spell=true status.end_event=true
    end
end
function card.aftercast(status,current_event,spell)
    if spell.english == card.item_use then
        card.item_use = 'none'
    end
end