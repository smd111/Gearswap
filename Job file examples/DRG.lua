...
window_hidden = true
-----------------------------------------------------------------------------------------------------------------------------------
jobneck = {neck={ name="Drachen Torque", augments={'STR+3','VIT+3',}},} --if using the conquest include put the neck that you want as your main neck when conquest neck is not needed
...
function get_sets()
    ...
    legs="Wool Hose",
    feet="Wool Socks",
    neck={ name="Drachen Torque", augments={'STR+3','VIT+3',}},
    waist="Corsette",
...
function mf_precast(spell,status,set_gear)
	...
	if pet.isvalid and spell.english == "Spirit Link" and pet.hpp >= 26 then
		status.end_spell=true
		status.end_event=true
		return
	end
