...
function ammo.to_bag(ammo)
    local find_bag_type = {} local find_ammo_type = {} local bag_type = 'none' local ammo_name = ""
    ...
    elseif ammo == "Antique Bullet" or ammo == "Atq. Bullet +1" or ammo:contains("旧式弾") then
        local number = tonumber(string.match(ammo, '%d')) or 0 local id = 5284 + number find_bag_type = res.items[id]
    elseif ammo:contains("Dogbolt") or ammo:contains("ドッグボルト") then
        local number = tonumber(string.match(ammo, '%d')) or 0 local id = 5278 + number find_bag_type = res.items[id]
    elseif ammo:contains("Crude Arrow") or ammo:contains("野矢") then
        local number = tonumber(string.match(ammo, '%d')) or 0 local id = 5270 + number find_bag_type = res.items[id]
    elseif ammo == "Old Arrow" or ammo:contains("古びた矢") then
        ...
        if find_ammo_type then
            if find_ammo_type.en:contains('Bolt') then bag_type = 'bolt quiver'
            elseif find_ammo_type.en:contains('Arrow') then bag_type = 'quiver'
            elseif find_ammo_type.en:contains('Bullet') then bag_type = 'bullet pouch'
            end
        end
    ...
    if not find_bag_type then return false else return find_bag_type end
end
...
function ammo.aftercast(status,current_event,spell) if spell.english == ammo.item_use then ammo.item_use = 'none' end end